...
// HARD RULE: never send a product button to Amazon search results or a guessed substitute.
// Exact Amazon /dp/ASIN links are added only from verifiedAmazon below.
for (const x of p) {
  const urls=[x.officialUrl,x.amazonUrl,x.buyUrl,x.url].filter(v=>typeof v==="string");
  if (urls.some(u=>/amazon\.com\/s(?:\?|\/)/i.test(u))) {
    if (typeof x.officialUrl==="string" && /amazon\.com\/s(?:\?|\/)/i.test(x.officialUrl)) x.officialUrl="";
    if (typeof x.amazonUrl==="string" && /amazon\.com\/s(?:\?|\/)/i.test(x.amazonUrl)) x.amazonUrl="";
    if (typeof x.buyUrl==="string" && /amazon\.com\/s(?:\?|\/)/i.test(x.buyUrl)) x.buyUrl="";
    if (typeof x.url==="string" && /amazon\.com\/s(?:\?|\/)/i.test(x.url)) x.url="";
    x.hasAmazon=false;
    x.amazonLinkType="unverified";
    x.commerceRoute="No verified exact purchase path";
  }
}
